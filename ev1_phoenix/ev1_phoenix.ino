...
#define boardColour 1 //0 = EVC01 REV1, 1 = EVC01 REV2, EVC01 REV3, 2 = NEW VER (WA)
    ...
    if (!isScanning) {
      ...
      if ((itIsTime(currentTimeExecte, previousTimeExecte, eventIntervalExecte)) && !inScanFunc) {
        ...
        if (!execRfidAuthenChargeButton || !execRfidAdd) {
          ...
          //
          //          if ((now.hour() > 25 || now.minute() > 60) && pingSuccess && (isInWebSite == 0)) {
          //            while (!timeClient.update()) {
          //              timeClient.forceUpdate();
          //            }
          //            ntpTimeTemp = timeClient.getFormattedDate();
          //            Serial.println(ntpTimeTemp);
          //            dateTempNTP = getValue(ntpTimeTemp, 'T', 0);
          //            Serial.println(dateTempNTP);
          //            timeTempNTP = getValue(ntpTimeTemp, 'T', 1);
          //            Serial.println(timeTempNTP);

          //            for (uint8_t i = 0; i < 3; i++) {
          //              timmingNTPTemp[i] = (getValue(dateTempNTP, '-', i)).toInt();
          //              Serial.print("DATE: ");
          //              Serial.println(timmingNTPTemp[i]);
          //            }
          //            for (uint8_t i = 0; i < 3; i++) {
          //              if (i == 2) {
          //                timmingNTPTemp[i + 3] = (getValue(timeTempNTP, ':', i).substring(0, getValue(timeTempNTP, ':', i).length() - 1)).toInt();
          //              }
          //              else {
          //                timmingNTPTemp[i + 3] = (getValue(timeTempNTP, ':', i)).toInt();
          //              }
          //              Serial.print("HOUR: ");
          //              Serial.println(timmingNTPTemp[i + 3]);
          //            }
          //            timetime.secoundNow = (getValue(timeTempNTP, ':', 2).substring(0, getValue(timeTempNTP, ':', 2).length() - 1)).toInt();
          //            timetime.minuiteNow = (getValue(timeTempNTP, ':', 1)).toInt();
          //            timetime.hoursNow = (getValue(timeTempNTP, ':', 0)).toInt();
          //            timetime.dateNow = (getValue(dateTempNTP, '-', 2)).toInt();
          //            timetime.monthNow = (getValue(dateTempNTP, '-', 1)).toInt();
          //            timetime.yearNow = (getValue(dateTempNTP, '-', 0)).toInt();
          //          }
          //          else {
