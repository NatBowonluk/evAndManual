...
void WiFiStationConnected(WiFiEvent_t event, WiFiEventInfo_t info) {
  ...
  //#if ISMQTT == 1
  //  mqtt.setServer(String(config.mqttip).c_str(), config.mqttport);
...
void WiFiGotIP(WiFiEvent_t event, WiFiEventInfo_t info) {
  ...
  //  pingSuccess = Ping.ping("www.google.com", 3);
}
...
void initwifiap() {
  ...
  uint16_t ip0 = getValue(String(config.ip), '.', 0).toInt();
  uint16_t ip1 = getValue(String(config.ip), '.', 1).toInt();
  uint16_t ip2 = getValue(String(config.ip), '.', 2).toInt();
  uint16_t ip3 = getValue(String(config.ip), '.', 3).toInt();
